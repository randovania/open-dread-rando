...
def apply_objective_patches(editor: PatcherEditor, configuration: dict):
    ...
    patch_text(
        editor, "RANDO_ARTIFACTS_ALL", r"All Metroid DNA acquired.|{c3}Raven Beak{c0} awaits you in {c5}Itorash{c0}."
    )

    access_point = copy.deepcopy(
    ...
    hints = {f"DIAG_ADAM_SHIP_2_PAGE_{i + 1}": hint for i, hint in enumerate(configuration["objective"]["hints"])}
    apply_text_patches(editor, hints)

    mlogs = {f"MLOG_ITEM_RANDO_ARTIFACT_{i + 1}": f"Acquired {r'{c1}'}Metroid DNA {i + 1}{r'{c0}'}" for i in range(12)}
