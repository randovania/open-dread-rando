...
def _modify_actor(
    ...
    if modifications:
        json_delta.patch(
            actor,
            [
                [modification["path"], modification["update_to"]]
                if "update_to" in modification
    ...
    if actor_groups is not None:
        ...
        not is_in_place
        and original_reference["scenario"] == new_reference["scenario"]
        and original_reference["actor_layer"] == new_actor_layer
    ):
